...
/// This crate has a runtime dependency on artifacts produced by the `rust-memleak-ebpf` crate.
...
/// crate's undeclared dependency on artifacts from `rust-memleak-ebpf` would cause build (and
...
/// performance; an environment variable is used to replace true builds of `rust-memleak-ebpf`
/// with stubs to preserve the property that code generation and linking (in
/// `rust-memleak-ebpf`) do not occur on metadata-only actions such as `cargo check` or `cargo
...
fn main() {
    ...
    let ebpf_package = packages
        .into_iter()
        .find(|Package { name, .. }| name == "rust-memleak-ebpf")
        .unwrap();
    ...
    if build_ebpf {
        ...
        // We have a build-dependency on `rust-memleak-ebpf`, so cargo will automatically rebuild us
        // if `rust-memleak-ebpf`'s *library* target or any of its dependencies change. Since we
        // depend on `rust-memleak-ebpf`'s *binary* targets, that only gets us half of the way. This
        ...
        let ebpf_target_dir = out_dir.join("rust-memleak-ebpf");
